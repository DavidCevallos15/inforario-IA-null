...
import { Calendar, X, Download, CheckCircle2, RefreshCw, AlertCircle } from 'lucide-react';
...
const CalendarModal: React.FC<CalendarModalProps> = ({ isOpen, onClose, onConfirm, schedule }) => {
  ...
  const calculateDates = () => {
    ...
    start.setDate(today.getDate() + ((1 + 7 - today.getDay()) % 7)); // Next Monday
  ...
  const handleGoogleSync = async () => {
    ...
    try {
      const result = await syncScheduleToCalendar(schedule, start, end);
      ...
      setSyncMessage('Error inesperado al sincronizar con Google Calendar.');
    } finally {
      setSyncing(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-surface-container-lowest rounded-3xl shadow-editorial border border-outline-variant/15 max-w-md w-full overflow-hidden">
        ...
        <div className="bg-surface-container p-5 border-b border-outline-variant/15 flex justify-between items-center">
          ...
          <button onClick={handleClose} className="text-on-surface-variant hover:text-on-surface bg-surface-container-high p-2 rounded-full transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="p-6">
          {syncSuccess ? (
            <div className="text-center py-4">
              ...
              <h3 className="text-lg font-bold text-on-surface mb-2">Sincronizado con exito</h3>
              <p className="text-sm text-on-surface-variant mb-6">{syncMessage}</p>
              <button 
                onClick={handleClose}
                className="w-full py-3 bg-primary text-on-primary font-bold rounded-xl hover:bg-primary/90 transition-all"
              >
                Cerrar
              </button>
            </div>
            ...
            <>
              <div className="mb-6 bg-primary/10 border border-primary/20 rounded-xl p-4">
                <h3 className="text-primary font-bold flex items-center gap-2 mb-2">
                  ...
                  Sincronizacion Universal
                </h3>
                <p className="text-sm text-on-surface-variant mb-3 leading-relaxed">
                  Exporta tu horario directamente a Google Calendar o descarga un archivo .ics compatible con cualquier calendario.
                </p>
                <p className="text-xs text-on-surface-variant border-l-2 border-primary/50 pl-2">
                  Tus materias se programaran automaticamente semana a semana hasta finalizar el ciclo academico.
                </p>
              </div>

              {gapiError && (
                <div className="mb-4 bg-error-container/30 border border-error/20 rounded-xl p-3 flex items-start gap-2">
                  <AlertCircle size={16} className="text-error shrink-0 mt-0.5" />
                  <p className="text-xs text-error">{gapiError}</p>
                </div>
              )}
              ...
              {syncMessage && !syncSuccess && (
                <div className="mb-4 bg-error-container/30 border border-error/20 rounded-xl p-3 flex items-start gap-2">
                  <AlertCircle size={16} className="text-error shrink-0 mt-0.5" />
                  <p className="text-xs text-error">{syncMessage}</p>
                </div>
              )}
              ...
              <div className="flex flex-col gap-3">
                ...
                >
                  ...
                  {syncing ? 'Sincronizando...' : gapiLoading ? 'Cargando...' : 'Agregar a Google Calendar'}
                </button>
